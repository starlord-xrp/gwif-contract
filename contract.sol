...
contract GaryWif is ERC20, Ownable {
    ...
    constructor() ERC20("Gary Wif", "gWIF") {
        uint256 totalSupply = 2025512100 ether;
        address garyWallet = 0x06A912e1d3C8De22bad1832E5A05ACaF58730463;
        ...
        _buyMarketingFee = 100; //1%
        _buyDevelopmentFee = 100; //1
        _buyLiquidityFee = 100; //1%
        buyTotalFees = _buyMarketingFee + _buyDevelopmentFee + _buyLiquidityFee;

        _sellMarketingFee = 100;
        _sellDevelopmentFee = 100;
        ...
        marketingWallet = garyWallet;
        developmentWallet = garyWallet;
        liquidityWallet = garyWallet;

        excludeFromFees(owner(), true);
        excludeFromFees(address(this), true);
        excludeFromFees(deadAddress, true);
        excludeFromFees(garyWallet, true);

        excludeFromMaxTransaction(owner(), true);
        excludeFromMaxTransaction(address(this), true);
        excludeFromMaxTransaction(deadAddress, true);
        excludeFromMaxTransaction(address(uniswapV2Router), true);
        excludeFromMaxTransaction(garyWallet, true);
    ...
    function make_liquidity_pool() public onlyOwner {
    ...
    function goGaryGo() public onlyOwner {
        require(!tradingActive, "Trading already active.");
        tradingActive = true;
        swapEnabled = true;
    }
    ...
    function setBuyFees(
        ...
        require(
            _marketingFee + _developmentFee + _liquidityFee <= 500,
            "ERC20: Must keep fees at 5% or less"
        );
